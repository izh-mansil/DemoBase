...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ЗапуститьИЗавершить") Тогда
...
Процедура ВыполнитьАнализКода(Команда)
	...
	Если Статус Тогда
		...
		ФормаРезультат = ОткрытьФорму("ВнешняяОбработка.АвторасстановкаШапокВПрограммныхМодулях.Форма.Результат", Параметр);
	КонецЕсли;
...
#Область ПроцедурыОбработкиМодулей

&НаСервере
Процедура УстановитьТекстПреамбулы()
	
	ДоступОбъект = РеквизитФормыВЗначение("Объект");
	ДоступОбъект.УстановитьТекстПреамбулы();
	ЗначениеВРеквизитФормы(ДоступОбъект, "Объект");
	
КонецПроцедуры

&НаСервере
Процедура ОбработатьТекстыМодулей()
	
	ДоступОбъект = РеквизитФормыВЗначение("Объект");
	ДоступОбъект.ОбработатьТекстыМодулей();
	ЗначениеВРеквизитФормы(ДоступОбъект, "Объект");
...
Функция АвтоформатироватьКод(ЗагрузитьКонфигурациюИзXML = Истина)
	...
	апк_Сообщить(НСтр("ru='Начало обработки текстов модулей.'"));
	
	ОбработатьТекстыМодулей();
	
	апк_Сообщить(НСтр("ru='Обработка текстов модулей завершена.'"));
